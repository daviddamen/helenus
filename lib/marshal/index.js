...
function getSerializer(type){
  if (Array.isArray(type)){
    ...
    if (type.isCollection) {
      var subser = type.subtype.map(function (val) {
        return getSerializer(val)
      });
      return TYPES[type.type.replace(/^\s+|\s+$/g,'')].ser(subser);
    } else {
...
function getDeserializer(type){
  if (Array.isArray(type)){
    ...
    if (type.isCollection) {
      var subde = type.subtype.map(function (val) {
        return getDeserializer(val)
      });
      return TYPES[type.type.replace(/^\s+|\s+$/g,'')].de(subde);    
    } else {
      return function(val) {
